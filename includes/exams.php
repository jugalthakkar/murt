...
class exam {
    ...
    {
        return WEB_ROOT . '#!/result/' . $this->Id . '/' . self::prettifyName() . '/';
//        return self::getURLforid($this->Id);
    }
    
    public static function prettify($str){
    ...
    private function prettifyName()    {
        return self::prettify($this->ExamName);
...
//        return urlencode(str_replace(' ', '-', trim($temp)));
    ...
    {
        if (defined('DISABLE_UPDATE') && DISABLE_UPDATE == true)
        {
            return;
        }
        $currTime = time();
