...
class meta{
    ...
    protected static function getDBFields() {
        return array('Id','MetaKey', 'MetaValue');
    }
    public $MetaKey;
    public $MetaValue;
    public static function getByKey($key) {
        $sql="SELECT * FROM " . self::getTableName() . " WHERE `MetaKey`='" . $key . "'";
        return array_shift(self::find_by_sql($sql));
    }
    //put your code here

       // Common DBEntityTemplateCode
    public $Id;

    public static function find_all($orderby='id ASC') {
        return self::find_by_sql("SELECT * FROM ".self::getTableName() . " ORDER BY {$orderby}");
    }
    ...
    public static function find_by_id($id=0) {
        $exam_array = self::find_by_sql("SELECT * FROM ".self::getTableName()." WHERE id={$id} LIMIT 1");
...
// return an array of attribute names and their values
        $attributes = array();
        foreach(self::getDBFields() as $field) {
